...
def drawGraphImages(df, tag_info):
    for i, tag in enumerate(df.columns):
        ...
        try: 
            ...
            ax1.set_title(label=f"[{i+1}] {tag} | mean={y.mean():.2f}, std={y.std():.2f}, min={y.min():.2f}, 25%={y.quantile(0.25):.2f}, 50%={y.quantile():.2f}, 75%={y.quantile(0.75):.2f}, max={y.max():.2f}", loc='left')
            ax1.plot(x, y, linestyle='solid', label='Operating', linewidth=2.0, color='#1d2088')
            ax1.grid(which = 'minor', axis = 'x', color = 'lightgrey', alpha = 0.8, linestyle = 'solid', linewidth = 0.5)
...
def drawGraphImages_interactive(df, tag_info):
    for i, tag in enumerate(df.columns):
        ...
        try:
            ...
            layout = go.Layout(
                title=f"[{i+1}] {tag_info.loc[i, 'Tag']} | {tag_info.loc[i, 'Description']} | {tag_info.loc[i, 'Unit']} | mean={y.mean():.2f}, std={y.std():.2f}, min={y.min():.2f}, 25%={y.quantile(0.25):.2f}, 50%={y.quantile():.2f}, 75%={y.quantile(0.75):.2f}, max={y.max():.2f}",
            ...
            layout = go.Layout(
                title=f"[{i+1}] {tag} | mean={y.mean():.2f}, std={y.std():.2f}, min={y.min():.2f}, 25%={y.quantile(0.25):.2f}, 50%={y.quantile():.2f}, 75%={y.quantile(0.75):.2f}, max={y.max():.2f}",
                xaxis=dict(title='Time'),
                yaxis=dict(title=tag),
                showlegend=True
            )
